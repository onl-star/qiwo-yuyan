...
object CustomConstant {
    ...
    const val CURRENT_RIME_DICT_DATA_VERSIOM = 2026061303
    ...
    fun stableSchemaForLegacyRime(schema: String): String = when {
        schema == SCHEMA_FROST || schema == SCHEMA_FROST_FULL -> SCHEMA_ZH_QWERTY
        schema == SCHEMA_FROST_T9 -> SCHEMA_ZH_T9
        schema == "rime_frost_double_pinyin" -> "${SCHEMA_ZH_DOUBLE_FLYPY}natural"
        schema.startsWith(SCHEMA_FROST_DOUBLE_PREFIX) ->
            SCHEMA_ZH_DOUBLE_FLYPY + schema.removePrefix(SCHEMA_FROST_DOUBLE_PREFIX)
        else -> schema
    }

    fun isUnsupportedFrostSchema(schema: String): Boolean =
        schema == SCHEMA_FROST ||
                schema == SCHEMA_FROST_FULL ||
                schema == SCHEMA_FROST_T9 ||
